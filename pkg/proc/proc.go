...
func PostInitializationSetup(p Process, debugInfoDirs []string) error {
	entryPoint, err := p.EntryPoint()
	if err != nil {
		return err
	}

	err = p.BinInfo().LoadBinaryInfo(p.Common().Path, entryPoint, debugInfoDirs)
	if err == nil {
	...
	createUnrecoveredPanicBreakpoint(p)

	panicpc, err := FindFunctionLocation(p, "runtime.startpanic", true, 0)
	if err == nil {
		bp, err := p.Breakpoints().SetWithID(-1, panicpc, p.WriteBreakpointFn())
		if err == nil {
...
func createUnrecoveredPanicBreakpoint(p Process) {
	panicpc, err := FindFunctionLocation(p, "runtime.startpanic", true, 0)
